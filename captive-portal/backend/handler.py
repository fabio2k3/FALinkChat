...
CAPTIVE_PORTAL_PATHS = {
    # Apple iOS / macOS
    # Espera: "Success" en el body. Si no lo recibe, abre portal.
    '/hotspot-detect.html': 'apple',
    '/library/test/success.html': 'apple',
    
    # Google Android
    # Espera: HTTP 204 No Content. Si recibe otra cosa, abre portal.
    '/generate_204': 'android',
    '/gen_204': 'android',
    '/connectivitycheck/gstatic/': 'android',
    
    # Microsoft Windows
    # Espera: "Microsoft Connect Test" en body. Si no, abre portal.
    '/connecttest.txt': 'windows',
    '/ncsi.txt': 'windows',
    '/redirect': 'windows',
    
    # Mozilla Firefox
    # Espera: "success" en body. Si no, abre portal.
...
def handle_request(raw_data: bytes, client_ip: str) -> bytes:
    """
    Función principal que procesa cada petición HTTP.
    ...
    for portal_path, os_type in CAPTIVE_PORTAL_PATHS.items():
        if path.startswith(portal_path) or path == portal_path:
            return handle_captive_detection(os_type, client_ip)
...
def handle_post_login(request: dict, client_ip: str) -> bytes:
    ...
    if check_user(user, password):
        ...
        try:
            ...
            print(f"[{client_ip}] IP autorizada en firewall")
